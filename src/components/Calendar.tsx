...
import { getUpcomingMeetings, selectEvent, selectRange, toggleModal, updateCalendarView, updateEvent } from 'store/reducers/calendar';
...
const Calendar = (props: any) => {
  ...
  const { calendarView, upcomingMeetings, isModalOpen, selectedRange } = useSelector((state) => state.calendar);
  const selectedEvent = useSelector((state) => {
    const { upcomingMeetings, selectedEventId } = state.calendar;
    if (selectedEventId) {
      return upcomingMeetings.find((event) => event.id === selectedEventId);
    }
    return null;
  });
  ...
  useEffect(() => {
    ...
    const eventCall = dispatch(getUpcomingMeetings());
    ...
    <Box sx={{ position: 'relative' }}>
      <CalendarStyled>
        ...
        <FullCalendar
          ...
          droppable
          selectable
          events={upcomingMeetings as EventSourceInput}
          ref={calendarRef}
          rerenderDelay={10}
          initialDate={date}
      ...
      >
        <AddEventForm event={selectedEvent} range={selectedRange} onCancel={handleModal} receiverEmail={props.receiverEmail} />
      </Dialog>
